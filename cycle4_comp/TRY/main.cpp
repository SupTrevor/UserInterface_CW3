...
void loadFolder(std::string loc) {

    qDebug() << "has loaded" << endl;
    folderWidget->getInfoIn( loc );
    ...
    for (int i = 0; i < folderWidget->buttons->size(); i++)
    {
        ...
        button->connect(button, SIGNAL(jumpTo2(TheButtonInfo*)), button, SLOT (addinQueue(TheButtonInfo *)));
    }
}
...
int main(int argc, char *argv[])
{
    ...
    //    folderWidget = new TheWidget();
        folderWidget = (TheWidget * )(w.get_folderWidget());
       // folderWidget = w.get_folderWidget();

//        folderWidget = new TheWidget(w.get_folderWidget());
//        folderWidget = (TheWidget * )folderWidget;
        folderWidget->init();
